...
String receivedWord;
void setup() {
  Serial.begin(9600); // Establece la velocidad de transmisión en 9600 bps (bits por segundo)
...
void loop() {
if (Serial.available()) {
    receivedWord = Serial.readString(); // Lee la palabra recibida por el puerto serial y almacénala en receivedWord
    Serial.println("Palabra recibida: " + receivedWord);
    text_ok = 1; // Establece el flag text_ok en 1 para activar la visualización de la palabra
  }

  currentMillis = micros();
  elapsed_loop_counter = currentMillis - previousMillis;
  delayTime = time_per_deg * 3.5; // Queremos 2 grados por cada línea de las letras

  if ((elapsed_loop_counter >= time_per_deg * 216) && (elapsed_loop_counter < time_per_deg * 217) && text_ok) {
    displayString(receivedWord.c_str(), delayTime);
    text_ok = 0;
  }
}
